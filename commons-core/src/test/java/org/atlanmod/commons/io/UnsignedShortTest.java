...
public class UnsignedShortTest {
    ...
    void testFromInt(int value) {
        UnsignedShort us = UnsignedShort.fromInt(value);
        assertThat(us.intValue()).isEqualTo(value);
    }
     ...
     */
    @ParameterizedTest
    @ValueSource(ints = {UnsignedShort.MIN_VALUE, UnsignedShort.MAX_VALUE, 1, UnsignedShort.MAX_VALUE - 1, 42})
    void testToBytesAndFromBytes(int value) {
        UnsignedShort expected = UnsignedShort.fromInt(value);
        byte[] bytes = expected.toBytes();
        UnsignedShort actual = UnsignedShort.fromBytes(bytes);
